...
import com.example.lezhin_webtoon_ipp.databinding.ItemWideRecyclerBinding

class HomeWideListAdapter : ListAdapter<WebtoonItem, HomeWideListAdapter.MyViewHolder>(
    diffUtil
) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MyViewHolder {
        return MyViewHolder(
            ItemWideRecyclerBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        )
    }
    ...
    inner class MyViewHolder(private var binding: ItemWideRecyclerBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(item: WebtoonItem, position: Int){
            //웹툰 이미지
            Glide.with(binding.itemWideImageview.context)
                .load(item.img)                   //이미지의 ur
                .error(R.drawable.ic_error)   //이미지 로드 중 에러 발생시 표시할 이미지
                .into(binding.itemWideImageview)           //이미지를 설정할 이미지뷰

            //웹툰 제목
            binding.itemWideTextview.text = item.title

            binding.itemWideLayout.setOnClickListener {
                val intent = Intent(binding.root.context, DetailActivity::class.java)
                intent.putExtra("URL", item.url)
                binding.root.context.startActivity(intent)
            }

            setMarginLayout(binding.itemWideLayout, position)
        }
    }
